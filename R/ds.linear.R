...
#'
#' According to \insertCite{walpole1993probability}{dsMiceClient}, the method of matrices consists in build the
...
#' @references
#' \insertRef{walpole1993probability}{dsMiceClient}
#' @export
...
ds.linear = function(formula, weight=1, type='combine', checks=FALSE, datasources=NULL){
  ...
  rownames(beta.reg) <- c("(Intercept)", indepVars)
    
  return(list(call=model.formula, coefficients=beta.reg))
