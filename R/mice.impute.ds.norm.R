...
#'}
#'
#'Using \code{mice.impute.ds.norm} for all columns emulates Schafer's NORM method (Schafer, 1997).
#'@references
...
#'@keywords datagen
#'@export
mice.impute.ds.norm <- function(y, ry, x, wy = NULL, ...) {
  if (is.null(wy)) wy <- !ry
  x <- cbind(1, as.matrix(x))
  parm <- .ds.norm.draw(y, ry, x, ...)
  return(x[wy, ] %*% parm$beta + rnorm(sum(wy)) * parm$sigma)
}

#' Draws values of beta and sigma by Bayesian linear regression for DataSHIELD
#'
...
#'
#'@aliases ds.norm.draw .ds.norm.draw
...
.ds.norm.draw <- function (y, ry, x, rank.adjust = TRUE, ...){
  ...
  beta.star <- p$c + (t(chol(mice:::sym(p$v))) %*% rnorm(ncol(x))) * sigma.star
...
#' Computes least squares parameters for DataSHIELD
#'
...
#' or Singular Value Decomposition. This function is internally called by .ds.norm.draw(),
...
#'@export
ds.estimice <- function(x, y, ls.meth = "qr", ridge = 1e-05, ...){
  ...
  if (ls.meth == "qr"){
    ...
    if(inherits(v, "try-error")){
      ...
      mice:::updateLog(out = mess, frame = 6)
