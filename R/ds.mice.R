...
ds.mice <- function(vars = NULL, m = 5, 
                    type = 'split',
                    method = NULL,
  ...
  #   cally <- call("miceDS", vars.temp)
  # } else {
  #   cally <- call("miceDS") 
  # }
  # result <- datashield.aggregate(datasources, cally)
  # 
  
  execution <- 1
  findNas <- ds.findNas(execution = execution, datasources = datasources)
  
  controlImputations <- length(findNas$naCols)
  
  #Build formula for regression
  #while(controlImputations != 0) {
    lstFormula <- NULL
    y <- findNas$naCols[[1]]
      #lstFormula <- rbind(lstFormula, paste0(variable, "~", ))
      formulas <- paste(paste0("D$", y), "~",
                         paste(paste0("D$", findNas$completeCols), collapse = "+"))
       
    impute <- ds.impute(formula = formulas, family = "gaussian", type=type, m=maxit, datasources=datasources)
    #controlImputations <- controlImputations - 1
  #}
    
    findNas2 <- ds.findNas(execution = 2, datasources = datasources)
    y2 <- findNas2$naCols[[1]]
    formulas2 <- paste(paste0("D$", y2), "~",
                      paste(paste0("D$", findNas2$completeCols), collapse = "+"))
    impute2 <- ds.impute(formula = formulas2, family = "gaussian", type=type, m=maxit, datasources=datasources)
    
    findNas3 <- ds.findNas(execution = 3, datasources = datasources)
    y3 <- findNas3$naCols[[1]]
    formulas3 <- paste(paste0("D$", y3), "~",
                        paste(paste0("D$", findNas3$completeCols), collapse = "+"))
    impute3 <- ds.impute(formula = formulas3, family = "gaussian", type=type, m=maxit, datasources=datasources)
    
  return(impute3)
  
  #############################
  # combinedPredMatrix <- merge(result[[1]]$predictorMatrix, result[[2:length(result)]]$predictorMatrix)
  # rownames(combinedPredMatrix) <- colnames(combinedPredMatrix)
  # 
  # combinedBlocks <- merge(result[[1]]$blocks, result[[2:length(result)]]$blocks)
  # 
  # formulas <- make.formulas(combinedPredMatrix, combinedBlocks)
  # 
  ...
  #                                      where = where, blocks = combinedBlocks)
  # method <- check.method(method = "pmm", vars = vars, where = where, 
  #                       blocks = combinedBlocks, defaultMethod = defaultMethod)
